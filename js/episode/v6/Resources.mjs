...
export class Resources {
    ...
    reset(){
        for (const resource_id of this.#resource_ids) {
            const resource = this.getResource(resource_id);

        // for (const resource of this.#resources) {
        // this.#resources.forEach((resource)=>{
            console.log('@@+ reset resource ======================================', resource.resource_category);

            if(resource instanceof Howl) {
                resource.stop();
                resource.volume(resource.defaultVolume);
            }else if(resource instanceof Image || resource instanceof Chara || resource instanceof PIXI.Container){
