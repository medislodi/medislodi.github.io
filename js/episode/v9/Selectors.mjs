...
export class Selectors {
    ...
    onClick(event=null, selected = 1) {
        ...
        // スキップ対策
        console.log('#$#$ selected', selected, this.selectedLog[this.#player.scenario_index]);
        if(this.#player.isSkip &&　this.selectedLog[this.#player.scenario_index] !== undefined) {
            selected = this.selectedLog[this.#player.scenario_index];
        }
        ...
        this.selectedLog[this.#player.scenario_index] = selected;
    ...
    set(number, text, jump_to, group_id) {
        ...
        if(scale < 1 && maxLength <= text.length) {
