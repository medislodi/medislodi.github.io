...
export class Player {
    ...
    #showStartButton() {
        ...
        document.getElementById("head-menu-buttons").scrollIntoView({behavior: "smooth", block: "start"});
    ...
    #load() {
        ...
        return new Promise((resolve, reject)=> {
            ...
            Object.entries(episode_json.contents.data).forEach(([key,value])=>{
                if(value.disable === "1") {console.log("!+ skip"); return ;}
                if(value.delay < 0) value.delay = 0;
                console.log('@@@ scenario', key, value);
    ...
    isEnableTap(scenario_index){
        if(this.finishFlags[scenario_index]['serif'] && this.finishFlags[scenario_index]['motion'] && !this.finishFlags[scenario_index]['taped']){
    ...
    enableTap(isSerif, scenario_index){
        ...
        console.log('@@+@@ check voice_for_replay.length',  this.voice_for_replay.length);
    ...
    next(){
        ...
        scenarios.forEach((origin)=>{
            ...
            if(this.isSkip && this.skipTarget !== this.scenario_index ) {
                if( !["jump","happy","panic","puru","angry","down"].includes(scenario.motion) && scenario.option !== "loop" ){
                    scenario.msec = 0;
                    scenario.delay = 0;
                }
            }
            ...
            let delay_promise = new Promise((resolve)=>{
                ...
                setTimeout(()=>{
                    Promise.all([
                        ...
                        this.#z(scenario),
                        this.#pivot(scenario),
                        this.#pivot_center(scenario),
                        this.#pivot_center_x(scenario),
                        this.#scale(scenario),
                        this.#scaleX(scenario),
                        this.#scaleY(scenario),
                        this.#alpha(scenario),
                        this.#wait(scenario),
                        ...
                        this.#e_play(scenario),
                        this.#e_stop(scenario),
                        this.#e_visible(scenario),
                        this.#e_vanish(scenario),
                        this.#e_goto_and_play(scenario),
                        this.#e_goto_and_stop(scenario),
                        this.#mask(scenario),
                        this.#m_remove(scenario),
                        this.#m_position(scenario),
                        this.#m_x(scenario),
                        this.#m_y(scenario),
                        this.#m_z(scenario)
                    ]).then(()=>{
    ...
    #motion(scenario){
        if(!scenario.motion) return new Promise((resolve)=>resolve());

        const chara = this.#resources.getResource(scenario.resource_id);
        console.log('@-@ motion', scenario, chara);
        // アーカイブ再現
        if(scenario.motion === "no_serif" && chara[scenario.motion]){
            console.log('@-@-@ motion no serif filter', scenario, chara);
            // フィルタをかける

            return Promise.all([
                this.#filter({
                    type: "filter",
                    msec: scenario.msec ?? 0,
                ...
                chara[scenario.motion].bind(chara)(scenario)
            ]);
        }else if(scenario.motion === "serif" && chara[scenario.motion]){
            this.#filter({
                type: "filter",
                msec: 0,
                resource_id: scenario.resource_id,
        ...
        // 元々の処理
        if(scenario.motion && chara && chara[scenario.motion]) {
    ...
    #wait(scenario){
        ...
        if(scenario.option !== "nonstop"){
            console.log('@-@-@ wait with cursor');
            this.#needCursor = true;
        }else{
            console.log('@-@-@ wait nonstop');
        }
    ...
    #serif(scenario){
        ...
        return new Promise((resolve)=>{
            promise.then(()=>{
                let promises = [];
                let name = this.#resources.getName(scenario.resource_id) ?? "";
                if(scenario.option && !['center','right','left','more-right','more-left','very-right','very-left'].includes(scenario.option)){
    ...
    #clone(scenario){
        if(scenario.type !== "clone") return new Promise((resolve)=>resolve());
        return this.#resources.imageCopy(scenario.resource_id, scenario.value, scenario.option);
    }

    #image(scenario){
        if(scenario.type !== "image") return Promise.resolve();
        let resource = this.#resources.getResource(scenario.resource_id);
        let property = JSON.parse(scenario.option === "" ? '{}' : scenario.option);
        if(resource === null) {
            console.warn('image resource not found', scenario.resource_id, scenario.value);
            return Promise.resolve();
        }
        resource.visible = property.visible ?? resource.visible;
        resource.x = property.x ?? resource.x;
        resource.y = property.y ?? resource.y;
        resource.zIndex = property.zIndex ?? resource.zIndex;
        resource.alpha = property.alpha ?? resource.alpha;
        resource.scale.x = property?.scale?.x ?? resource.scale.x;
        resource.scale.y = property?.scale?.y ?? resource.scale.y;
        resource.width = property.width ?? resource.width;
        resource.height = property.height ?? resource.height;
        // container.filters = resource.filters;
        resource.defaultAlpha = property.alpha ?? resource.alpha;
    ...
    #w_mode(scenario){
        if(scenario.type !== "w_mode" || scenario.resource_id !== 2) return Promise.resolve();
        this.textWindow.mode = scenario.value;
        return Promise.resolve();
    }
    ...
    #volume(scenario){
        ...
        let fade_time = scenario.msec >= 0 ? scenario.msec : 500;
        /** @var {Howl} sound */
    ...
    #voice(scenario){
        const voice = this.#resources.getResource(scenario.sound);
        console.log('@@+@@ voice', voice !== null, scenario.sound, this.isSkip, this.skipTarget, this.scenario_index);
        if(voice !== null) this.voice_for_replay.push(scenario.sound);
        console.log('@@+@@ voice_for_replay.length',  this.voice_for_replay.length);
        if(voice !== null && (!this.isSkip || this.skipTarget === this.scenario_index)) {
            voice.mute(!this.setting.voice);
            if (!isNaN(scenario.volume) && scenario.volume > 0) voice.volume(scenario.volume);
    ...
    #y(scenario){
        ...
        if(scenario.msec < 0) scenario.msec = 0;
        const resource = this.#resources.getResource(scenario.resource_id);
        console.log('y preset', resource, scenario);
    ...
    #pivot(scenario){
        if(scenario.type !== "pivot") return new Promise((resolve)=>resolve());
        const resource = this.#resources.getResource(scenario.resource_id);
        // resource が Chara の時は Chara.anime のpivotを中心にする
        if(resource instanceof Chara){
            resource.anime.pivot.x = scenario.value.split(':')[0];
            resource.anime.pivot.y = scenario.value.split(':')[1];
            resource.base.x = scenario.value.split(':')[0];
            resource.base.y = scenario.value.split(':')[1];

        }

        return new Promise((resolve)=> {
            resource.pivot.x = scenario.value.split(':')[0];
            resource.pivot.y = scenario.value.split(':')[1];
            resolve();
        });
    }

    #pivot_center(scenario){
        if(scenario.type !== "pivot_center") return new Promise((resolve)=>resolve());
        const resource = this.#resources.getResource(scenario.resource_id);
        // resource が Chara の時は Chara.anime のpivotを中心にする
        if(resource instanceof Chara){
            resource.anime.pivot.x = resource.anime.width / 2;
            resource.anime.pivot.y = resource.anime.height / 2;
             return new Promise((resolve)=> {
                resolve();
            });
        }
        console.log('pivot_center', resource);
        return new Promise((resolve)=> {
            resource.pivot.x = resource.width / 2;
            resource.pivot.y = resource.height / 2;
            resolve();
        });
    }


    #pivot_center_x(scenario){
        if(scenario.type !== "pivot_center_x") return new Promise((resolve)=>resolve());
        const resource = this.#resources.getResource(scenario.resource_id);
        // resource が Chara の時は Chara.anime のpivotを中心にする
        if(resource instanceof Chara){
            console.log('######## pivot_center_x before', resource.anime.pivot.x);
            resource.anime.pivot.x = resource.anime.width;
            console.log('######## pivot_center_x after', resource.anime.pivot.x);
            // resource.base.x = resource.anime.width;
            return new Promise((resolve)=> {
                resolve();
            });
        }
    ...
    #scaleX(scenario){
        if(scenario.type !== "scale_x") return new Promise((resolve)=>resolve());
        if(scenario.msec < 0) scenario.msec = 0;
        const resource = this.#resources.getResource(scenario.resource_id);
        return this.#animator.scaleToX(resource, scenario.value, scenario.msec, scenario.option ?scenario.option: "liner");
    }

    #scaleY(scenario){
        if(scenario.type !== "scale_y") return new Promise((resolve)=>resolve());
        if(scenario.msec < 0) scenario.msec = 0;
        const resource = this.#resources.getResource(scenario.resource_id);
        return this.#animator.scaleToY(resource, scenario.value, scenario.msec, scenario.option ?scenario.option: "liner");
    }
    ...
    #filter(scenario) {
        ...
        switch (option.type){
            case "color":
                ...
                // console.log('@--@ filter color', filter,option.method,option.anime_param,option.params);
            ...
            case "blur":
                if("blur" in resource.activeFilters){
                    ...
                    filter = new PIXI.filters.BlurFilter(...option.params);
    ...
    #toast(scenario){
        ...
        this.ui.toast().innerHTML = scenario.value;
    ...
    #e_play(scenario){
        if(scenario.type !== "e_play") return new Promise((resolve)=>resolve());
        if(scenario.msec < 0) scenario.msec = 0;
        const effect = this.#resources.getResource(scenario.resource_id);
        effect.visible = true;
        ...
        return new Promise((resolve)=>{
            effect.movie.addEventListener("finish",(eve)=>{
                eve.currentTarget.removeEventListener("finish");
                resolve();
            });
            effect.movie.play();
        });
    ...
    #e_goto_and_play(scenario){
        if(scenario.type !== "e_goto_and_play") return new Promise((resolve)=>resolve());
        if(scenario.msec < 0) scenario.msec = 0;
        const effect = this.#resources.getResource(scenario.resource_id);
        effect.visible = true;
        ...
        return new Promise((resolve)=>{
            ...
            effect.movie.gotoAndPlay(scenario.value);
        });
    }

    #e_stop(scenario){
        if(scenario.type !== "e_stop") return new Promise((resolve)=>resolve());
        if(scenario.msec < 0) scenario.msec = 0;
        const effect = this.#resources.getResource(scenario.resource_id);
        effect.movie.stop();
        return new Promise((resolve)=>resolve());
    ...
    #e_visible(scenario){
        if(scenario.type !== "e_visible") return new Promise((resolve)=>resolve());
        if(scenario.msec < 0) scenario.msec = 0;
        const effect = this.#resources.getResource(scenario.resource_id);
        effect.visible = true;
        return new Promise((resolve)=>resolve());
    ...
    #e_vanish(scenario){
        if(scenario.type !== "e_vanish") return new Promise((resolve)=>resolve());
        if(scenario.msec < 0) scenario.msec = 0;
        const effect = this.#resources.getResource(scenario.resource_id);
        effect.movie.stop();
    ...
    #e_goto_and_stop(scenario){
        if(scenario.type !== "e_goto_and_stop") return new Promise((resolve)=>resolve());
        if(scenario.value < 1) scenario.value = 1;
        const effect = this.#resources.getResource(scenario.resource_id);
        effect.visible = true;
        ...
        effect.movie.gotoAndStop(scenario.value);
        return new Promise((resolve)=>resolve());
    }


    #mask(scenario){
        if(scenario.type !== "mask") return new Promise((resolve)=>resolve());
        const resource = this.#resources.getResource(scenario.resource_id + "m");
        if(!resource instanceof Mask){
            throw new Error("Maskを設定しようとしましたが、Maskクラスではありません。");
        }
        return new Promise((resolve)=> {
            // scenario.value から resource_idを取得
            const target = this.#resources.getResource(scenario.value);
            console.log("#mask", scenario, target);
            resource.setTarget(target, scenario.option ?? 0);
            resolve();
        });
    }

    #m_remove(scenario){
        if(scenario.type !== "m_remove") return new Promise((resolve)=>resolve());
        const resource = this.#resources.getResource(scenario.resource_id + "m");
        if(!resource instanceof Mask){
            throw new Error("Maskを設定しようとしましたが、Maskクラスではありません。");
        }
    ...
    #m_position(scenario){
        if(scenario.type !== "m_position") return new Promise((resolve)=>resolve());
        const resource = this.#resources.getResource(scenario.resource_id + "m");
        if(!resource instanceof Mask){
            throw new Error("Maskを設定しようとしましたが、Maskクラスではありません。");
        }
        console.log("#m_position", scenario.option);
        const option = JSON.parse(scenario.option === "" ? "{}" : scenario.option);
        return resource.positionTo(scenario.value, scenario.msec, option?.line_break ? Number(option.line_break) : 3);
    }

    #m_x(scenario){
        if(scenario.type !== "m_x") return new Promise((resolve)=>resolve());
        const resource = this.#resources.getResource(scenario.resource_id + "m");
        if(!resource instanceof Mask){
            throw new Error("Maskを移動しようとしましたが、Maskクラスではありません。");
        }
        return resource.xTo(scenario.value, scenario.msec);
    }

    #m_y(scenario){
        if(scenario.type !== "m_y") return new Promise((resolve)=>resolve());
        const resource = this.#resources.getResource(scenario.resource_id + "m");
        if(!resource instanceof Mask){
            throw new Error("Maskを移動しようとしましたが、Maskクラスではありません。");
        }
        return resource.yTo(scenario.value, scenario.msec);
    }

    #m_z(scenario){
        if(scenario.type !== "m_z") return new Promise((resolve)=>resolve());
        const resource = this.#resources.getResource(scenario.resource_id + "m");
        if(!resource instanceof Mask){
            throw new Error("Maskを移動しようとしましたが、Maskクラスではありません。");
        }
        return resource.zIndex = scenario.value;
    }
...
class FilterUpdater{
    ...
    set param(value){
        console.log('@--@ filter update', this.method, this.target_param,this.params[this.target_param], value);
